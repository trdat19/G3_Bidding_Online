...
public class AuctionDao {
    ...
    public boolean insertAuction(Auction auction) {
        ...
        try (Connection con = DBconnection.getInstance().getConnection();
            ...
            // nếu object chưa set min_increment thì cho về 1.0
            // khớp với default của DB
            ps.setDouble(5, auction.getMin_increment() > 0 ? auction.getMin_increment() : 1.0);

            // buy_now_price trong DB cho phép null
            // vì class đang dùng double nên quy ước:
            // <= 0 thì coi như không có giá mua ngay
            ps.setObject(6,
            ...
            // lấy id tự tăng từ DB gán lại vào object
            if (rowsAffected > 0) {
    ...
    // dùng khi cần lấy đúng 1 phiên đấu giá
    ...
    // dùng để test hoặc admin xem tất cả phiên đấu giá
    ...
    // dùng khi seller muốn xem các phiên mình đã tạo
    ...
    // ví dụ OPEN, PREPARING, CLOSED, CANCELLED
    ...
    // dùng khi chỉ muốn đổi trạng thái mà không sửa cột khác
    ...
    // dùng để tránh mở nhiều phiên đấu giá cùng lúc cho 1 item
    ...
    // ánh xạ 1 dòng DB thành object Auction
    // dùng chung cho các hàm SELECT để tránh lặp code
