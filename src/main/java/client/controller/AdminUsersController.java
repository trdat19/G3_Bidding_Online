...
import shared.dto.request.admin.AdminGetUsersRequest;
import shared.dto.response.BaseResponse;
import shared.dto.response.admin.AdminUsersResponse;
...
public class AdminUsersController {
    ...
    private void loadUsers() {
        AdminGetUsersRequest request = new AdminGetUsersRequest();
        BaseResponse response = ClientNetworkService.getInstance().sendRequest(request);

        if (response instanceof AdminUsersResponse adminResponse) {
            List<UserDTO> user = adminResponse.getUsers();
