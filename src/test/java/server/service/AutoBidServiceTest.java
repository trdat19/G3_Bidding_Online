...
public class AutoBidServiceTest {
    ...
    public void reactToIncomingBidLocked_placesBidAboveSecondRule() {
        ...
        when(bidService.placeAutoBid(5L, 10L, new BigDecimal("170.00"))).thenReturn(true);
    ...
    @Test
    @DisplayName("switchAutoBid - bat/tat rule cua bidder")
    public void switchAutoBid_success() {
        when(autoBidRuleDAO.switchActiveStateRule(5L, 10L, false)).thenReturn(true);

        assertTrue(autoBidService.switchAutoBid(5L, 10L, false));

        verify(autoBidRuleDAO).switchActiveStateRule(5L, 10L, false);
    }
